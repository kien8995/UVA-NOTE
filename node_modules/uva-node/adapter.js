const colors = require('colors');
const fs = require('fs');
const path = require('path');
const util = require('./util');
var Table = require('cli-table');
// Maps from typeName to class function
var subClasses = {}; // private static field
var normNames = {}; // normalize the type name

module.exports = (function(){
    // constructor
    function cls(app, acct)
    {
        // public instance method
        this.send = function(probNum, filePath, callback){
            // suf includes the dot
            var suf = path.extname(filePath);
            if (suf.length <= 1)
            {
                callback({message: "Cannot auto-detect language"});
                return;
            }

            suf = suf.substring(1).toLowerCase();
            this._send(probNum, filePath, suf, callback);
        };

        this.account  = function(){
            return acct;
        };
    }

    cls.normalizeType = function(s){
        return normNames[s.toLowerCase()];
    };

    // public static method
    cls.create = function(app, acct){
        var clsFn = subClasses[acct.type().toLowerCase()];
        if (clsFn) return new clsFn(app, acct);
        return null;
    };

    const STATUSES = {
        STATUS_ERROR         : {label: "       Submit error >.<   ".bold,      color: "red"},
        STATUS_QUEUE_ERROR   : {label: "        Can't queue :|    ".bold,   color: "red"},
        STATUS_IN_QUEUE      : {label: "       -_- in queue -_-   ".bold,      color: "yellow"},
        STATUS_COMPILE_ERROR : {label: "       Compile error *_-  ".bold,   color: "yellow"},
        STATUS_RESTRICTED_FN : {label: "    Restricted function :(".bold,   color: "yellow"},
        STATUS_RUNTIME_ERROR : {label: "       Runtime error :(   ".bold,   color: "cyan"},
        STATUS_OUTPUT_LIMIT  : {label: "       Output limit -_-   ".bold,  color: "yellow"},
        STATUS_TIME_LIMIT    : {label: "        Time limit -_-    ".bold,    color: "blue"},
        STATUS_MEM_LIMIT     : {label: "       Memmory limit :(   ".bold,     color: "yellow"},
        STATUS_WRONG_ANS     : {label: "       Wrong answer! :(   ".bold,     color: "red"},
        STATUS_PRESENTATION  : {label: "    Presentation error! :(".bold,  color: "yellow"},
        STATUS_ACCEPTED      : {label: "         Accepted! :D     ".bold,      color: "green"},
        STATUS_UNKNOWN       : {label: "        Waiting... >.<    ".bold,     color: "magenta"},
    };

    for (var key in STATUSES)
    {
        var status = STATUSES[key];
        status.coloredLabel = status.label[status.color];
        cls[key] = status;
    }

    return cls;
})();

/*
 * Auto load the subclasses
 */
(function(){
    var files = fs.readdirSync(__dirname);
    for (var i=0; i < files.length; i++)
    {
        var match = /^adapter(\w+)/i.exec(files[i]);
        if (!match) continue;
        var modName = match[1];
        var lower = modName.toLowerCase();
        normNames[lower] = modName;
        subClasses[lower] = require('./'+files[i]);
    }
})();
